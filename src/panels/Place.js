...
const Place = ({ item, order, onIncrementPosition, onDecrementPosition, area, history }) => {
  ...
  const isOrderExists = checkOrderExisting(order, item.id)
    console.log(order)

    const sendOrder = () => {
      if (isOrderExists) {
          history.push(`/basket/${area.id}/${item.id}`)
      } else {
  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
          ...
          <button className="Place__order" onClick={sendOrder}>
