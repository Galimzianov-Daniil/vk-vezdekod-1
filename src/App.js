			...
			<Switch>
				...
				<Route path="/basket/:areaId/:itemId" exact>
					<Basket
						order={order}
						foodAreas={FOOD_AREAS}
						setParams={(time, status, itemId) => {
							const nextActiveOrdersProps = (JSON.parse(localStorage.getItem('activeOrderParams')) || {});
							nextActiveOrdersProps[itemId] = { timeParam: time, isSelfServingParam: status }
							localStorage.setItem('activeOrderParams', JSON.stringify(nextActiveOrdersProps));

						}}
						setDefaultOrderParams={(itemId) => {
							const nextActiveOrdersProps = (JSON.parse(localStorage.getItem('activeOrderParams')) || {});
							nextActiveOrdersProps[itemId] = { isSelfServingParam: false, timeParam: "faster" }
							localStorage.setItem('activeOrderParams', JSON.stringify(nextActiveOrdersProps));
						}}
						getOrderParams={(itemId) => {

							return (JSON.parse(localStorage.getItem('activeOrderParams')) || {})[itemId];
						}}
					/>
				</Route>
				<Route
